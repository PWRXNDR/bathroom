...
| Минимум 30 FPS | Выполнено с запасом | Около 60 FPS без ограничения при render scale 1.25, финальная частота намеренно ограничена |
...
Отдельное полотенце уменьшено с 1.47 MiB до 0.24 MiB и содержит 60 476 треугольников. Значение 340 149 в таблице относится только к основной модели ванной. Вместе с отдельным полотенцем вся runtime сцена содержит 400 625 треугольников, 78 render meshes и 49 KTX2 текстур.
...
- около 60 FPS без ограничения частоты при render scale 1.25
...
RTX 5060 Laptop является достаточно мощной дискретной видеокартой. Этот результат показывает запас на high-end ноутбуке, но сам по себе не является доказательством такой же производительности на iPad-class устройствах. Дополнительно production версия проверялась на Samsung Galaxy S23 Ultra: сцена работала со стабильной частотой без заметных провалов при навигации. Это полезная проверка реального мобильного устройства, но не замена полноценной матрице мобильных GPU.

`stats.js` измеряет время работы кадра на CPU, это не GPU timestamp. Изолированный per-pass breakdown не приводится, потому что отдельные GPU timestamp measurements для GTAO, SSR, SSGI и TAA ещё не были собраны. Отключение только визуального contribution не убирает стоимость pass, поэтому такие цифры были бы вводящими в заблуждение. Для строгого профилирования следующим шагом нужны WebGPU timestamp queries или отдельные pipeline variants без каждого эффекта.
